...
int main(int argc, char *argv[]) {
  ...
  ***************************************************/
  ...
  h_vec_t<double> *value = new h_vec_t<double>[uniq_keys.size()];
  h_vec_t<int> *column = new h_vec_t<int>[uniq_keys.size()];
  h_vec_t<int> *row_csr = new h_vec_t<int>[uniq_keys.size()];

  int **pntrb = new int *[uniq_keys.size()];
  int **pntre = new int *[uniq_keys.size()];

  for (int i = 0; i < uniq_keys.size(); ++i) {
    pntrb[i] = new int[num_feat_2];
    pntre[i] = new int[num_feat_2];
  }
  ...
  for (int i = 0; i < uniq_keys.size(); ++i) {
    ...
    CompressMatrix(value[i], column[i], row_csr[i],
                   affinity_blocks.data() + i * len_distance_2, num_feat_2,
                   num_feat_2);

//    totalnnz += value[i].size(); 
    pntrb[i] = row_csr[i].data();
    pntre[i] = row_csr[i].data() + 1;
  }
 ...
 // std::cout << std::endl;
 // std::cout << "key size:" << uniq_keys.size() << std::endl;
 // std::cout << "nnz: " << totalnnz << std::endl;
 // std::cout << std::endl;

  // for (int i = 0; i < uniq_keys.size(); ++i) {
  //   std::cout << uniq_keys[i] << std::endl;
  //   for (int j = 0; j < len_distance_2; ++j) {
  //     std::cout << affinity_blocks[i * len_distance_2 + j] << " ";
  //   }
  //   std::cout << std::endl;
  // }
  // std::cout << std::endl;
  ...
  //          std::cout << value[i][j] << " " << row[i][j] << std::endl;
  //      }
  //      std::cout << std::endl;
  //  }
 ...
 // std::cout << std::endl;
 // for (int i = 0; i < uniq_keys.size(); ++i) {
 //   for (int j = 0; j < num_feat_2; ++j) {
 //     std::cout << pntrb[i][j] << " " << pntre[i][j] << std::endl;
 //   }
 //   std::cout << std::endl;
 // }
 // std::cout << std::endl;
  ...
  for (int iter = 0; iter < num_iters; ++iter) {
    for (int i = 0; i < num_keys; i++) {
      for (int j = 0; j < h_keys_idcs[i].size(); j++) {
        ...
        mkl_dcsrmv("N", &num_feat_2, &num_feat_2, &one, "G**C", value[i].data(),
                   column[i].data(), pntrb[i], pntre[i],
                   h_eigen_old.data() + col * num_feat_2, &one,
                   h_eigen_new.data() + row * num_feat_2);
      }
    }
